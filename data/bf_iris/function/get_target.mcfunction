...
# To tell where a player is looking, anchoring to the eye position is needed: execute as <player> at @s anchored eyes positioned ^ ^ ^ run function bf_iris:get_target
# Targeted block/entity information is sent to storage (see output below) but a marker with tag 'bf_iris.ray' is also present in the targeted block after running this function
#
# @public
# @context an entity, their eye position, and their rotation
# @reads
#   storage bf_iris:settings
#       TargetEntities: byte
...
#           Defaults to "#bf_iris:shape_groups/air"
...
#       TargetedEntity: int
#           The ID of the targeted entity on objective bf_iris.entity_id
...
tag @e remove bf_iris.targeted_entity
tag @e remove bf_iris.potential_target
kill @e[type=minecraft:marker, tag=bf_iris.targeted_block]
scoreboard players reset * bf_iris.id

data modify storage bf_iris:output TargetType set value "NONE"
data remove storage bf_iris:output TargetedBlock
data remove storage bf_iris:output TargetedEntity
data remove storage bf_iris:output TargetPosition
data remove storage bf_iris:output Distance
data remove storage bf_iris:output TargetedBox
data remove storage bf_iris:output TargetedFace

scoreboard players set $depth bf_iris 0
scoreboard players set $min_distance bf_iris 2147483647
scoreboard players set $max_entity_id bf_iris 0
scoreboard players set $total_distance bf_iris 0

# Get initial position/rotation
execute summon minecraft:marker run function bf_iris:get_position/main

# Start the loop
tag @s add bf_iris.executing
execute store result score $max_depth bf_iris run data get storage bf_iris:settings MaxRecursionDepth
return run function bf_iris:raycast/loop
